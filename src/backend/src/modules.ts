    ...
    try {
        ...
        values = values.map(value => {
            if (value) {
                ...
                switch (explicit_type) { // This handles <number>i <number>f <number>b, etc.
                    case 'i': return v_int;
                    case 'f': return Math.fround(v_float);
                    case 'b': return parseInt(v);
                    default: break;
                }
...
export async function GetAvatarDetails(avi_id: string, cookie: { auth?: string, twoFactorAuth?: string }, getListingData = false): Promise<$VRChatAPIAvatarData | undefined> {
    if (!cookie.auth) { console.log(chalk.bgRed(`[GetAvatarDetails] Failed because 'auth' was not specified.`)); return; }
    if (!cookie.twoFactorAuth) { console.log(chalk.bgYellow(`[GetAvatarDetails] May fail because 'twoFactorAuth' was not specified.`)); }
