...
export class VRC_OSC_INTERFACE extends MessageListeners { // TODO: genericize this
    ...
    Create(CONFIG: $VRC_OSC_INTF_ARGS) {
        ...
        this.server.on('message', (...args) => {
            ...
            LOGGING && console.log('⬇ [VRChat => OSC_INTF]', address, values); // FLOATS MUST BE BETWEEN LIKE 0.01 AND SOMETHING ELSE TO AVOID OVERFLOWING XD

            this.HandleData(this, address, values);
            this.UN_ACK.size > 0 && this.SetAcknowledged(MD5({ address, values }));
        });

        this.server.on('close', () => { this.init = false; console.log('[OSC_INTF] Server closed.') });
    ...
    async SendValue(address: string, ...values: any[]): Promise<boolean> {
        ...
        this.UN_ACK.size >= 256 && this.UN_ACK.shift();

        await this.client.send(address, ...values);

        LOGGING && console.log('⬆ [OSC_INTF => VRChat]', address, values);

        return new Promise<boolean>(resolve => {
            let hash = MD5({ address, values });
            this.SetUnacknowledged(hash, resolve);
            setTimeout(() => { /* this.RemoveUnacknowledged(hash); */ resolve(false); }, 1000 * 0.1); // ~64 requests/ms (6.6k/0.1s) to a loopback in optimal conditions
        });
    }
}